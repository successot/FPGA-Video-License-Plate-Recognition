...
`define UD #1
// -----------------------------------------------------------------------------
// MES50HP Stage7F13 4CH 1080P QUAD STABLE
//
...
// -----------------------------------------------------------------------------
module mes50hp_stage7f13_4ch_1080p_quad_top #(
...
wire        ch0_vsync, ch1_vsync, ch2_vsync, ch3_vsync;
wire        ch0_de,    ch1_de,    ch2_de,    ch3_de;
wire [15:0] ch0_rgb565,ch1_rgb565,ch2_rgb565,ch3_rgb565;
wire [31:0] ch0_udp_packet_count, ch1_udp_packet_count, ch2_udp_packet_count, ch3_udp_packet_count;
wire [31:0] ch0_fpgv_packet_count,ch1_fpgv_packet_count,ch2_fpgv_packet_count,ch3_fpgv_packet_count;
wire [31:0] ch0_frame_count, ch1_frame_count, ch2_frame_count, ch3_frame_count;

fpgv_gmii_line_to_vin #(.LOCAL_MAC(48'h02_00_00_00_50_01), .LOCAL_IP(32'hC0A8_0164), .UDP_PORT(16'd5000), .SRC_W(SRC_W), .SRC_H(SRC_H), .OUT_W(SRC_W), .OUT_H(SRC_H)) u_fpgv_ch0 (
    .clk(p0_clk), .rst_n(p0_rstn & sys_rst_n), .gmii_rxd(p0_rxd), .gmii_rx_dv(p0_dv), .gmii_rx_er(p0_er),
    .vin_vsync(ch0_vsync), .vin_de(ch0_de), .vin_data(ch0_rgb565), .udp_packet_pulse(), .fpgv_packet_pulse(), .frame_written_pulse(), .overflow_pulse(),
    .udp_packet_count(ch0_udp_packet_count), .fpgv_packet_count(ch0_fpgv_packet_count), .frame_written_count(ch0_frame_count), .overflow_count(), .last_frame_id(), .last_packet_id(), .last_width(), .last_height(), .debug_state());
fpgv_gmii_line_to_vin #(.LOCAL_MAC(48'h02_00_00_00_50_01), .LOCAL_IP(32'hC0A8_0164), .UDP_PORT(16'd5000), .SRC_W(SRC_W), .SRC_H(SRC_H), .OUT_W(SRC_W), .OUT_H(SRC_H)) u_fpgv_ch1 (
    .clk(p1_clk), .rst_n(p1_rstn & sys_rst_n), .gmii_rxd(p1_rxd), .gmii_rx_dv(p1_dv), .gmii_rx_er(p1_er),
    .vin_vsync(ch1_vsync), .vin_de(ch1_de), .vin_data(ch1_rgb565), .udp_packet_pulse(), .fpgv_packet_pulse(), .frame_written_pulse(), .overflow_pulse(),
    .udp_packet_count(ch1_udp_packet_count), .fpgv_packet_count(ch1_fpgv_packet_count), .frame_written_count(ch1_frame_count), .overflow_count(), .last_frame_id(), .last_packet_id(), .last_width(), .last_height(), .debug_state());
fpgv_gmii_line_to_vin #(.LOCAL_MAC(48'h02_00_00_00_50_01), .LOCAL_IP(32'hC0A8_0164), .UDP_PORT(16'd5000), .SRC_W(SRC_W), .SRC_H(SRC_H), .OUT_W(SRC_W), .OUT_H(SRC_H)) u_fpgv_ch2 (
    .clk(p2_clk), .rst_n(p2_rstn & sys_rst_n), .gmii_rxd(p2_rxd), .gmii_rx_dv(p2_dv), .gmii_rx_er(p2_er),
    .vin_vsync(ch2_vsync), .vin_de(ch2_de), .vin_data(ch2_rgb565), .udp_packet_pulse(), .fpgv_packet_pulse(), .frame_written_pulse(), .overflow_pulse(),
    .udp_packet_count(ch2_udp_packet_count), .fpgv_packet_count(ch2_fpgv_packet_count), .frame_written_count(ch2_frame_count), .overflow_count(), .last_frame_id(), .last_packet_id(), .last_width(), .last_height(), .debug_state());
fpgv_gmii_line_to_vin #(.LOCAL_MAC(48'h02_00_00_00_50_01), .LOCAL_IP(32'hC0A8_0164), .UDP_PORT(16'd5000), .SRC_W(SRC_W), .SRC_H(SRC_H), .OUT_W(SRC_W), .OUT_H(SRC_H)) u_fpgv_ch3 (
    .clk(p3_clk), .rst_n(p3_rstn & sys_rst_n), .gmii_rxd(p3_rxd), .gmii_rx_dv(p3_dv), .gmii_rx_er(p3_er),
    .vin_vsync(ch3_vsync), .vin_de(ch3_de), .vin_data(ch3_rgb565), .udp_packet_pulse(), .fpgv_packet_pulse(), .frame_written_pulse(), .overflow_pulse(),
    .udp_packet_count(ch3_udp_packet_count), .fpgv_packet_count(ch3_fpgv_packet_count), .frame_written_count(ch3_frame_count), .overflow_count(), .last_frame_id(), .last_packet_id(), .last_width(), .last_height(), .debug_state());
...
fram_buf_4ch_quad #(
    ...
    .ch0_vin_clk(p0_clk), .ch0_wr_fsync(ch0_vsync), .ch0_wr_en(ch0_de), .ch0_wr_data(ch0_rgb565),
    .ch1_vin_clk(p1_clk), .ch1_wr_fsync(ch1_vsync), .ch1_wr_en(ch1_de), .ch1_wr_data(ch1_rgb565),
    .ch2_vin_clk(p2_clk), .ch2_wr_fsync(ch2_vsync), .ch2_wr_en(ch2_de), .ch2_wr_data(ch2_rgb565),
    .ch3_vin_clk(p3_clk), .ch3_wr_fsync(ch3_vsync), .ch3_wr_en(ch3_de), .ch3_wr_data(ch3_rgb565),
...
wire [15:0] hdmi_rgb565 = video_show_pix ? quad_rgb565 : test_rgb565;

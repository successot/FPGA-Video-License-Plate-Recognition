...
module lcd_driver(
    ...
    output          lcd_pclk  ,  //LCD sample clock
    output          lcd_frame_start,
    output          lcd_rgb565_valid,
    output  [15:0]  lcd_rgb565_data
    );
...
assign lcd_pclk = lcd_clk;  //RGB LCD sample clock

// UDP export signals, aligned to the registered LCD RGB565 output.
// lcd_frame_start is a one-clock pulse at the first active output pixel.
assign lcd_frame_start   = ((h_cnt == (h_sync + h_back)) && (v_cnt == (v_sync + v_back))) ? 1'b1 : 1'b0;
assign lcd_rgb565_valid  = lcd_de;
assign lcd_rgb565_data   = lcd_rgb;

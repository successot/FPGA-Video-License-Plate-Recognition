    ...
    if(!rst_n) begin
        ...
        case(next_state)
            st_idle     : begin
                if(trig_tx_en) begin
                    ...
                    if(des_ip != 32'd0)
                        ...
                        ip_head[4] <= DES_IP;       
                    //16位源端口号：1234  16位目的端口号：5001                      
                    ip_head[5] <= {16'd5001,16'd5001};  
                    //16位udp长度，16位udp校验和              
            ...
            st_tx_data  : begin
				...
				/*
				* tx_data is 32-bit wide.
				* tx_req requests the next 32-bit word from the upstream FIFO.
				*
				* Important:
				* The old code requested one more word near the end of every UDP packet.
				* That caused the packetizer FIFO to be over-read by 4 bytes per UDP packet,
				* so the next packet header "FPGV" drifted by 4 bytes each packet.
				*/
				...
				if(data_cnt < tx_data_num) begin
					case(tx_bit_sel)
						...
						2'd2: begin
							...
							/*
							* Request next 32-bit word only if after finishing current
							* 4-byte word, there will still be payload bytes to send.
							*
							* At tx_bit_sel == 2, current word bytes sent/being sent are:
							*   byte0, byte1, byte2
							* byte3 will be sent next cycle.
							* So the current word covers bytes up to data_cnt + 1.
							* Another word is needed only when:
							*   data_cnt + 2 < tx_data_num
							*
							* Equivalent practical condition:
							*   data_cnt + 16'd2 < tx_data_num
							*/
							if(data_cnt + 16'd2 < tx_data_num)
								tx_req <= 1'b1;
						end
						...
						2'd3: begin
							...
							if(data_cnt + 16'd1 >= tx_data_num) begin
								skip_en    <= 1'b1;
								data_cnt   <= 16'd0;
								tx_bit_sel <= 2'd0;
							end

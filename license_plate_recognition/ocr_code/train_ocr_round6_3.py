...
CONFIG = {
    "stage_name": "ocr_round6_3_filtered_refine_from_c",
    ...
    # reuse round6.2 mined candidates, but filter aggressively in-code
    ...
    # mostly clean, tiny hard
    "clean_train_total_per_epoch": 118000,
    "hard_train_total_per_epoch": 3000,
    ...
    "hard_max_wan_ratio": 0.46,

    # filtered hard buckets only
    "hard_category_ratio": {
        "province_to_wan_targeted": 0.18,
        "first_char_confusion": 0.48,
        "tail_related": 0.18,
        "nonfirst_core_single_char": 0.12,
        "wan_stabilizer": 0.04,
    },

    # lighter weights than round6.2
    "hard_sample_weight": 1.03,
    "non_wan_weight": 1.04,
    "wan_weight": 0.99,
    "province_to_wan_weight": 1.15,
    "first_char_confusion_weight": 1.20,
    "tail_related_weight": 1.05,
    "nonfirst_core_single_char_weight": 1.00,
    "wan_stabilizer_weight": 1.02,
    "max_sample_weight": 1.6,

    # only mild augmentation
    "augment": True,
    "aug_brightness": (0.75, 1.25),
    "aug_contrast": (0.75, 1.25),
    "aug_color": (0.85, 1.08),
    "aug_affine_prob": 0.18,
    "aug_blur_prob": 0.12,
    "aug_downup_prob": 0.14,
    "aug_jpeg_prob": 0.12,
    "aug_occlude_prob": 0.08,
    "aug_noise_prob": 0.10,
...
def hamming_if_same_len(a: str, b: str):
    if len(a) != len(b):
        return None
    return sum(1 for x, y in zip(a, b) if x != y)


def keep_round6_3_row(row: Dict) -> bool:
    cat = row.get("category") or ""
    gt = row.get("gt") or ""
    pred = row.get("pred") or ""
    pos_raw = row.get("pos")
    pos = None
    if pos_raw not in (None, "", "nan"):
        try:
    ...
    # keep all high-value first-char buckets
    if cat in {"province_to_wan_targeted", "first_char_confusion", "wan_stabilizer"}:
        return True

    # tail bucket: keep only precise tail-missing or very confident same-len pos6 single-char
    if cat == "tail_related":
        if pred == gt[:-1]:
            return True
        hd = hamming_if_same_len(gt, pred)
        if len(gt) == len(pred) and hd == 1 and pos == 6 and conf <= 0.58:
            return True
        return False

    # nonfirst bucket: keep only same-len single-char at pos2/3/4 with moderate confidence and no first-char change
    if cat == "nonfirst_core_single_char":
        hd = hamming_if_same_len(gt, pred)
        if len(gt) == len(pred) and hd == 1 and pos in {2, 3, 4} and gt[:1] == pred[:1] and conf <= 0.74:
            return True
        return False

    # multi-char same-len: pause completely in round6.3
    if cat == "multi_char_same_len":
        return False
...
class OCRDataset(Dataset):
    ...
    def _affine(self, im, rng):
        return transforms.functional.affine(
            ...
            translate=(int(im.width * rng.uniform(-0.03, 0.03)), int(im.height * rng.uniform(-0.05, 0.05))),
            scale=rng.uniform(0.96, 1.04),
            shear=[rng.uniform(-2.5, 2.5), 0.0],
    ...
    def _occlude(self, im, rng):
        ...
        overlay = Image.new("RGB", (band_w, h), tuple(rng.randint(80, 170) for _ in range(3)))
        im = im.copy()
...
def build_sample_weights(items: List[SampleItem], cfg: Dict) -> torch.Tensor:
    ...
    for x in items:
        ...
        elif x.category == "first_char_confusion":
            w *= cfg["first_char_confusion_weight"]
        elif x.category == "tail_related":
            w *= cfg["tail_related_weight"]
        elif x.category == "nonfirst_core_single_char":
            w *= cfg["nonfirst_core_single_char_weight"]
...
def main():
    ...
    summary = {
        ...
        "notes": {
            "round6_3_filtered_refine": [
                "base checkpoint is C (round6.1 best)",
                "keep high-value first-char buckets",
                "tail bucket is heavily narrowed",
                "multi_char_same_len is paused",
                "nonfirst_core_single_char keeps only same_len pos2/3/4 moderate-confidence rows",
            ]
        },
    }

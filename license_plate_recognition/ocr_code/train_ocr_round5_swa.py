...
CONFIG = {
    "stage_name": "round5_ultra_conservative_swa_from_round4",
...
def evaluate(model, loader, charset, device, amp_on, cfg, beam_decoder=None, force_beam: Optional[bool] = None):
    model.eval()
...
def main():
    ...
    if cfg.get("use_swa", False):
        ...
        print(f"SWA 从 epoch {cfg['swa_start_epoch']} 开始")
    ...
    print(f"\n{'='*60}")
    print(f"开始 Ultra Conservative + SWA ({cfg['epochs']} epochs)")
    print(
        f"init_ckpt={cfg['init_ckpt']}\n"
        f"head_lr={cfg['head_lr']}, backbone_lr={cfg['backbone_lr']}, "
        f"freeze_backbone_epochs={cfg['freeze_backbone_epochs']}, "
        f"swa_start_epoch={cfg['swa_start_epoch']}"
    )
    print(f"{'='*60}")
    ...
    if swa_model is not None:
        print("\n===== SWA BN 更新 =====")
        swa_model = swa_model.cpu().to(device)

        try:
            _, bn_loader, _ = build_epoch_data(
                train_sources,
                min(cfg["train_total_samples_per_epoch"], 5000),
                cfg["train_source_ratio_end"],
                charset,
                cfg,
                seed=cfg["seed"] + 99,
            ...
            print(f"  SWA BN 更新失败: {e}，跳过")

        _, val_loader_swa, val_counts_swa = build_epoch_data(
            val_sources,
            cfg["val_total_samples"],
            cfg["val_source_ratio"],
            charset,
        ...
        swa_metrics_greedy = evaluate(
            swa_model, val_loader_swa, charset, device, amp_on, cfg, beam_decoder, force_beam=False
        )
        print("\n===== SWA Greedy 评估 =====")
        print(f"  Val: {val_counts_swa}")
        print(f"  decode_mode={swa_metrics_greedy['decode_mode']}")
        print(f"  acc={swa_metrics_greedy['acc']:.4f} char_acc={swa_metrics_greedy['char_acc']:.4f}")
        for src, acc in swa_metrics_greedy["src_acc"].items():
            print(f"    {src}: {acc:.4f} ({swa_metrics_greedy['src_total'].get(src, 0)} samples)")

        swa_composite_greedy = sum(swa_metrics_greedy["src_acc"].values()) / max(len(swa_metrics_greedy["src_acc"]), 1)
        ...
        swa_ckpt = {
            ...
            "charset": charset.chars,
            "metrics": {
                "acc": swa_metrics_greedy["acc"],
                "composite": swa_composite_greedy,
                "decode_mode": "greedy",
            },
        }
        torch.save(swa_ckpt, save_dir / "best_swa.pt")
        print("  已保存 best_swa.pt")

        if cfg.get("use_beam_at_final", True) and beam_decoder is not None:
            _, val_loader_swa_beam, val_counts_swa_beam = build_epoch_data(
                val_sources,
                cfg["val_total_samples"],
                cfg["val_source_ratio"],
                charset,
            ...
            swa_metrics_beam = evaluate(
                swa_model, val_loader_swa_beam, charset, device, amp_on, cfg, beam_decoder, force_beam=True
            )
            print("\n===== best_swa.pt Beam 评估 =====")
            print(f"  Val: {val_counts_swa_beam}")
            print(f"  decode_mode={swa_metrics_beam['decode_mode']}")
            print(f"  acc={swa_metrics_beam['acc']:.4f} char_acc={swa_metrics_beam['char_acc']:.4f}")
            for src, acc in swa_metrics_beam["src_acc"].items():
                print(f"    {src}: {acc:.4f} ({swa_metrics_beam['src_total'].get(src, 0)} samples)")

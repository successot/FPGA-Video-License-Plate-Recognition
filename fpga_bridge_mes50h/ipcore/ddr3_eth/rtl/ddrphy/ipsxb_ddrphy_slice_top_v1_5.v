...
module ipsxb_ddrphy_slice_top_v1_5 #(
  ...
  input  [4:0]                      mc_rl                    ,
  input  [2*MEM_DQS_WIDTH-1:0]      init_read_clk_ctrl       ,
...
localparam CA_GROUP4_REG   =  0;
  ...
  wire [MEM_DQS_WIDTH-1:0]        samp_win_valid         ; 
  wire [8*MEM_DQS_WIDTH-1:0]      samp_win_size          ;
  wire [8*MEM_DQS_WIDTH-1:0]      samp_center_position   ;
...
always @(posedge ddrphy_clkin or negedge logic_ck_rstn) begin
    if(!logic_ck_rstn) begin
        init_pls    <= 1'b0;
        init_pls_r1 <= 1'b0;
        init_pls_r2 <= 1'b0;
    end
...
assign  logic_rstn_pos = init_pls_r1 & (~init_pls_r2);

always @(posedge ddrphy_clkin or negedge ddrphy_rst_n) begin
    if(!ddrphy_rst_n)
        ck_delay_wait_cnt <= 5'd0;
    else if(ck_delay_wait_cnt == 5'd24)
        ck_delay_wait_cnt <= ck_delay_wait_cnt;
    else if(wrlvl_ck_dly_start)
        ck_delay_wait_cnt <= ck_delay_wait_cnt + 1;
    else
        ck_delay_wait_cnt <= 5'd0;
end
...
begin
    if(!logic_ck_rstn)
      ck_dly_set_bin         <= 8'h0;
    else if(logic_rstn_pos)
      ck_dly_set_bin         <= init_ck_dly_step;
    else if(wrlvl_ck_dly_start & (ck_delay_wait_cnt == 5'd24))
...
generate
  for(gen_d=0; gen_d<MEM_DQS_WIDTH; gen_d=gen_d+1) begin   : i_dqs_group
    ipsxb_ddrphy_data_slice_v1_5 #(
      ...
      .mc_rl                     (mc_rl                                       ),         
      .init_read_clk_ctrl        (init_read_clk_ctrl[2*gen_d+1:2*gen_d]       ),
      ...
      .ck_delay_complete         (ck_delay_complete                           ),
      ...
      .samp_win_valid            (samp_win_valid[gen_d]                       ),
      .samp_win_size             (samp_win_size[8*gen_d+7:8*gen_d]            ),
      .samp_center_position      (samp_center_position[8*gen_d+7:8*gen_d]     ),

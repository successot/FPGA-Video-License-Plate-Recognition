...
// -----------------------------------------------------------------------------
// MES50H 阶段3：UDP 图像 payload 提取与缓存写入验证模块
// -----------------------------------------------------------------------------
// 目标：
//   1. 继续复用阶段2已经验证通过的 GMII RX / UDP / FPGV 解析链路。
//   2. 解析 FPGV 包头，并提取 payload[32] 之后的真实图像数据。
//   3. 对每个 UDP 包统计 payload_len 字节，默认每包 1024 字节。
//   4. 对同一 frame_id 内 packet_id 连续性进行检查。
//   5. 对一整帧累计 payload 字节数，默认 800*480*2 = 768000 字节。
//   6. 保存当前帧开头 8 个 RGB565 采样 word，便于 ILA 观察 payload 是否真实变化。
//   7. 本阶段只做 payload 缓存写入验证，不接 DDR3，不接 LCD。
// -----------------------------------------------------------------------------
module fpgv_stage3_payload_buffer_check #(
    ...
    // ATK 侧 checksum 算法没有在 MES50H 侧最终确认，默认只计算并输出，不参与 LED5 错误判定。
    parameter        ENABLE_CHECKSUM_ERROR = 1'b0
)(
    input             clk,
    input             rst_n,
    ...
    output reg [15:0] last_checksum16,
    output reg [15:0] packet_checksum_calc,
    output reg [15:0] expected_packet_id,
    output reg [31:0] frame_payload_byte_count,
    output reg [15:0] packet_payload_byte_count,
    output reg [7:0]  last_version,
    output reg [7:0]  last_mode,
    output reg [7:0]  last_pixel_format,

    output reg [15:0] sample_word0,
    output reg [15:0] sample_word1,
    output reg [15:0] sample_word2,
    output reg [15:0] sample_word3,
    output reg [15:0] sample_word4,
    output reg [15:0] sample_word5,
    output reg [15:0] sample_word6,
    output reg [15:0] sample_word7,

    output reg        packet_seq_ok,
    output reg        payload_write_active,
...
always @(posedge clk or negedge rst_n) begin
    if(!rst_n) begin
        ...
        payload_write_active      <= 1'b0;

        raw_frame_pulse           <= 1'b0;
        udp5000_pulse             <= 1'b0;
        fpgv_packet_pulse         <= 1'b0;
        payload_byte_valid        <= 1'b0;
        packet_payload_done_pulse <= 1'b0;
        frame_payload_done_pulse  <= 1'b0;
        ...
        packet_payload_done_count <= 32'd0;
        ...
        last_checksum16           <= 16'd0;
        packet_checksum_calc      <= 16'd0;
        frame_payload_byte_count  <= 32'd0;
        packet_payload_byte_count <= 16'd0;
        last_version              <= 8'd0;
        last_mode                 <= 8'd0;
        ...
        S_FRAME: begin
            if(!gmii_rx_dv) begin
                ...
                // payload[28:29] = checksum16
                // payload[32...] = image payload
                ...
                if(byte_idx == udp_base_idx + 11'd36) last_checksum16[15:8] <= gmii_rxd;
                if(byte_idx == udp_base_idx + 11'd37) last_checksum16[7:0]  <= gmii_rxd;

                // FPGV 头部最后一个字节是 payload[31]，下一字节 payload[32] 开始是真实图像数据。
                if(byte_idx == udp_base_idx + 11'd39) begin
                    packet_payload_byte_count <= 16'd0;
                    packet_checksum_calc      <= 16'd0;
                    payload_write_active      <= fpgv_ok;
                    ...
                    if(fpgv_ok) begin
                        ...
                        if(!stream_started) begin
                            // 第一次进入时可能正好位于一帧中间，所以先建立基准，不马上报错。
                            stream_started            <= 1'b1;
                            ...
                            // 新 frame_id 出现，重新开始累计一帧 payload 字节数。
                            current_frame_id          <= last_frame_id;
                            packets_seen_in_frame     <= 16'd1;
                            expected_packet_id        <= last_packet_id + 16'd1;
                            frame_payload_byte_count  <= 32'd0;
                ...
                // payload[32] 之后是真实图像数据。这里等效于“写入缓存”的字节有效脉冲。
                if(packet_header_accept &&
                   (byte_idx >= image_data_start_idx) &&
                   (packet_payload_byte_count < last_payload_len)) begin

                    payload_byte_valid        <= 1'b1;
                    packet_payload_byte_count <= packet_payload_byte_count + 16'd1;
                    frame_payload_byte_count  <= frame_payload_byte_count + 32'd1;
                    packet_checksum_calc      <= packet_checksum_calc + {8'd0, gmii_rxd};

                    // 保存当前帧最开头 8 个 RGB565 word 采样，后续可接 ILA。
                    if(frame_payload_byte_count < 32'd16) begin
                        case(frame_payload_byte_count[3:0])
                        4'd0:  sample_word0[15:8] <= gmii_rxd;
                        4'd1:  sample_word0[7:0]  <= gmii_rxd;
                        4'd2:  sample_word1[15:8] <= gmii_rxd;
                        4'd3:  sample_word1[7:0]  <= gmii_rxd;
                        4'd4:  sample_word2[15:8] <= gmii_rxd;
                        4'd5:  sample_word2[7:0]  <= gmii_rxd;
                        4'd6:  sample_word3[15:8] <= gmii_rxd;
                        4'd7:  sample_word3[7:0]  <= gmii_rxd;
                        4'd8:  sample_word4[15:8] <= gmii_rxd;
                        4'd9:  sample_word4[7:0]  <= gmii_rxd;
                        4'd10: sample_word5[15:8] <= gmii_rxd;
                        4'd11: sample_word5[7:0]  <= gmii_rxd;
                        4'd12: sample_word6[15:8] <= gmii_rxd;
                        4'd13: sample_word6[7:0]  <= gmii_rxd;
                        4'd14: sample_word7[15:8] <= gmii_rxd;
                        4'd15: sample_word7[7:0]  <= gmii_rxd;
                        default: begin
                            sample_word0 <= sample_word0;
                        end
                        endcase
                    end

                    // 当前包最后一个 payload 字节。
                    if(packet_payload_byte_count == (last_payload_len - 16'd1)) begin
                        packet_payload_done_pulse <= 1'b1;
                        packet_payload_done_count <= packet_payload_done_count + 32'd1;
                        payload_write_active      <= 1'b0;
                        packet_header_accept      <= 1'b0;

                        if(ENABLE_CHECKSUM_ERROR &&
                           ((packet_checksum_calc + {8'd0, gmii_rxd}) != last_checksum16)) begin
                        ...
                        if(last_packet_id == (last_packet_total - 16'd1)) begin
                            if(((frame_payload_byte_count + 32'd1) == EXPECTED_FRAME_BYTES) &&
                               current_frame_seq_ok &&
                               (last_packet_total == EXPECTED_PACKET_TOTAL)) begin
                                frame_payload_done_pulse <= 1'b1;
...
always @(posedge clk or negedge rst_n) begin
    if(!rst_n) begin
        ...
        debug_led[2] <= payload_byte_valid;            // 正在提取图像 payload
        debug_led[3] <= |packet_payload_done_count[7:0];// 已完成至少一个包的 payload 提取
        debug_led[4] <= |complete_frame_count[7:0];    // 已完成至少一帧 768000 字节 payload
        debug_led[5] <= (|seq_error_count[7:0]) |
                        (|length_error_count[7:0]) |
                        (|rx_error_count[7:0]) |
                        (|checksum_error_count[7:0]);  // 错误指示
